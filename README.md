...
**매장별 월별 매출 분석**
   - 매장별로 월별 매출 데이터를 집계하여, 어떤 매장이 언제 더 많은 매출을 기록했는지 파악합니다.
   
**특정 배우가 출연한 영화의 매출 기여도 분석**
   - 특정 배우가 출연한 영화들이 전체 매출에서 차지하는 비율을 계산하여, 배우의 영화 선택이 매출에 미치는 영향을 분석합니다.
   
**가장 신속한 대여 및 반환 서비스 제공 매장 분석**
   - 각 매장에서 대여 후 반환까지 걸린 평균 시간을 계산하여, 서비스 속도가 가장 빠른 매장을 파악합니다.
   
**대여되지 않은 영화 목록 찾기**
   - 대여되지 않은 영화를 찾아, 불필요한 재고를 줄이거나 프로모션 전략을 세우는 데 활용할 수 있습니다.
   
**고객의 활동성 분석**
   - 고객별 대여 횟수와 결제 금액을 기반으로 VIP 고객을 식별하여, 타겟 마케팅 전략을 세웁니다.
   
**특정 카테고리의 대여 트렌드 분석**
   - 특정 영화 장르의 대여 트렌드를 분석하여, 장르별로 어떤 월에 인기 있었는지를 파악합니다.
   
**특정 기간 동안 대여된 영화 목록**
   - 특정 기간 동안 대여된 영화와 대여 횟수를 분석하여, 인기 있는 영화를 확인하고 적절한 재고 관리 전략을 수립합니다.
   
**고객의 평균 대여 간격 분석**
   - 고객별 평균 대여 간격을 계산하여, 자주 대여하는 고객을 파악하고 이를 통해 맞춤형 서비스를 제공합니다.
