...
여기서, 
- \( z = w_0 + w_1 x_1 + w_2 x_2 + \dots + w_n x_n \)는 선형 방정식입니다.
- \( w_0, w_1, \dots, w_n \)은 모델의 파라미터(weights)입니다.
- \( x_1, x_2, \dots, x_n \)은 입력 변수(feature)입니다.
- \( \sigma(z) \)는 **시그모이드 함수**의 출력으로, 이 값은 0과 1 사이의 확률 값을 가집니다.
...
여기서:
- \( m \)은 샘플의 수
- \( y^{(i)} \)는 \( i \)-번째 샘플의 실제 클래스 값 (0 또는 1)
- \( h(x^{(i)}) = \sigma(z^{(i)}) \)는 \( i \)-번째 샘플에 대한 모델의 예측 확률
