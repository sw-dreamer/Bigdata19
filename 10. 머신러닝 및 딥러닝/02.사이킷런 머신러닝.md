...
X_train, X_test, y_train, y_test =train_test_split(
    X # 분리할 데이터
    ,y # 분리할 원래 답
    ,test_size=0.3 # 학습에 사용할 데이터를 전체의 70%로 만들어라
    ,random_state=1 # 시드 고정
    ,stratify=y # 계층화 : y는 True와 같음, random으로 분리 할 시 특정 레이블은 조금만 갖고 오고 어느 레이블은 많이 갖고 올 수 있는 것을 방지하는 방법 예를 들어 0,1,2번 레이블이 있다면 0번에서 (1-test_size)*100%를 1번에서 (1-test_size)*100%를 2번에서 (1-test_size)*100% 데이터를 가지고 오게 하는 방법
)
