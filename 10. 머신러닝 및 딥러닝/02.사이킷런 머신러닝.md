...
$$
\gamma : \text{전체 정규화 강도를 조절하는 하이퍼파라미터}
$$

$$
\alpha : \text{L1과 L2 정규화의 비율을 조절하는 하이퍼파라미터 (0 \leq \alpha \leq 1)}
$$

$$
\alpha = 1 \quad \text{이면 라소 회귀(L1)}
$$

$$
\alpha = 0 \quad \text{이면 릿지 회귀(L2)}
$$

$$
0 < \alpha < 1 \quad \text{이면 엘라스틱넷}
$$
