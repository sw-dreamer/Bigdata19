...
$$
- **Loss(w)** : 원래의 손실 함수 (예: 평균 제곱 오차, MSE)
- **\(\lambda_1 \sum |\mathbf{w}|\)** : L1 정규화(라소) → 일부 가중치를 0으로 만들어 변수 선택
- **\(\lambda_2 \sum \mathbf{w}^2\)** : L2 정규화(릿지) → 모든 가중치를 줄여 다중공선성 완화
$$
...
$$
**\(\gamma\)** : 전체 정규화 강도를 조절하는 하이퍼파라미터
**\(\alpha\)** : L1과 L2 정규화의 비율을 조절하는 하이퍼파라미터 (0 ≤ \(\alpha\) ≤ 1)
\(\alpha = 1\)이면 **라소 회귀**(L1)
\(\alpha = 0\)이면 **릿지 회귀**(L2)
\(0 < \alpha < 1\)이면 **엘라스틱넷**
$$
