...
'''
드롭아웃(Dropout)은 신경망 모델을 훈련할 때 **과적합(overfitting)**을 방지하는 중요한 기법입니다. 과적합은 모델이 훈련 데이터에 너무 잘 맞추어져서 새로운 데이터(테스트 데이터)에 대한 일반화 성능이 떨어지는 현상입니다. 드롭아웃은 이를 방지하기 위해 다음과 같은 방식으로 동작합니다:
