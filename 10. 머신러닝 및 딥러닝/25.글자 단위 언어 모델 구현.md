...
for i, (seq,target) in enumerate(seq_dataset):
    print('='*50)
    print(f'입력데이터 : {repr(char_array[seq])}')
    print(f'출력데이터 : {repr(char_array[target])}')
    if i==1:
        break
```
![image](https://github.com/user-attachments/assets/be0cc854-c9d5-4d4d-95a1-87ad6e10cfdb)
