...
class AdalineSGD:
    ...
    def partial_fit(self, X, y):
        ...
        if y.ravel().shape[0] > 1:  # 다수의 샘플에 대해 학습, ravel 함수는 주로 NumPy 라이브러리에서 사용되는 함수로, 다차원 배열을 1차원 배열로 평탄화(flatten)하는 데 사용
            for xi, target in zip(X, y):
    ...
    def _shuffle(self, X, y):
        ...
        X, y : 섞인 데이터와 레이블 반환
        '''
        r = self.rgen.permutation(len(y))  # 무작위 순서 생성
        return X[r], y[r]  # 섞은 데이터와 레이블 반환

    def _initialize_weights(self, m):
        '''가중치를 랜덤한 작은 값으로 초기화'''
        self.rgen = np.random.RandomState(self.random_state)  # 난수 생성기 초기화 (시드 설정)
        self.w_ = self.rgen.normal(loc=0.0, scale=0.01, size=m)  # 평균 0, 표준편차 0.01로 가중치 초기화
