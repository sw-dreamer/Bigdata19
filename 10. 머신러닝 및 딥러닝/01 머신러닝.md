...
class AdalineGD:
    ...
    def fit(self, X, y): # X : 학습 데이터, y : 원래 답
        ...
        self.b_ = np.float_(0.)  # 바이어스(bias)는 0으로 초기화, numpy 2.x는 np.float_ 말고 np.float64를 사용해야 함
        self.losses_ = []  # 각 반복에서의 손실값을 저장할 리스트
...
```
![image](https://github.com/user-attachments/assets/010f0ec8-5fc2-4a46-83f3-a6caac7c00f4)
