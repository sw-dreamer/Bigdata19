...
# 함수 정의 : model, num_epochs, train_dl,valid_dl,loss_fn,optimizer
def train(model, num_epochs, train_dl,valid_dl,loss_fn,optimizer):
    device=torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    # model => gpu 보내기
    model=model.to(device=device)  
    loss_hist_train=[0]*num_epochs
    acc_hist_train=[0]*num_epochs # gpu로 보내려면 list를 tensor로 바꾸고 to로 보내야한다.
    ...
    for epoch in range(num_epochs):
        ...
        with torch.no_grad():
            ...
            print(f'epoch : {epoch+1}, 학습 정확도 : {acc_hist_train[epoch]:.4f}, 검증 정확도 : {acc_hist_valid[epoch]:.4f} ')
