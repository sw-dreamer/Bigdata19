...
for epoch in range(num_epochs):
    acc_train,loss_train=train(model=model,dataloader=train_dl,optimizer=optimizer,loss_fn=loss_fn)
    acc_valid,loss_valid=evaluate(model=model,dataloader=valid_dl,loss_fn=loss_fn)
    print(f'에포크 : {epoch}, 정확도 : {acc_train:.4f}, 검증 정확도: {acc_valid}')
```
